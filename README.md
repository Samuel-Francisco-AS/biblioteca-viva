...
F0, F1, F2 e F3 — Câmera e interação mobile — da FUNDAÇÃO do novo mundo estão concluídos. O novo mundo continua definido como uma experiência 3D real com apresentação ortográfica/2.5D. **Three.js está aprovado como renderer da Fundação**, usando integração direta com React, `WebGLRenderer`, `OrthographicCamera` e GLTF/GLB. **F4 — Contrato experimental de assets 3D está em andamento**: F4-A/B/C/D/E estão concluídas experimentalmente. Em F4-E, a variante experimental Poly Haven 512 reduziu payload e a estimativa estrutural, preservou o contrato técnico e passou no gate humano com leve desfoque não bloqueante para o uso pretendido; isso não altera asset/runtime registrado nem cria budget ou pipeline definitivo. F4-F é o próximo checkpoint; F5–F6 permanecem futuros. A FUNDAÇÃO inteira ainda não está concluída.
