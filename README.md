...
F0–F4 da FUNDAÇÃO do novo mundo estão concluídas. O novo mundo continua definido como uma experiência 3D real com apresentação ortográfica/2.5D. **Three.js está aprovado como renderer da Fundação**, usando integração direta com React, `WebGLRenderer`, `OrthographicCamera` e GLTF/GLB. **F4 consolidou o contrato experimental de assets 3D**: autoria editável, normalização antes do runtime, materiais PBR relevantes, ownership/lifecycle experimental e custo medido; isso não cria budget, asset/runtime registrado, pipeline produtivo definitivo ou mundo final. **F5 — performance e validação Android física — é a próxima fase.** F6 permanece posterior. A FUNDAÇÃO inteira ainda não está concluída.
