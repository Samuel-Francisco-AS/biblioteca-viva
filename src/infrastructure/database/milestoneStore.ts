  ...
  implements MilestoneProcessor, MilestoneRepository
{
  ...
  async process(event: DomainEvent): Promise<readonly ReachedMilestone[]> {
    ...
    try {
      const [storedEntries, totalSessions, totalNotes, totalQuotes, reached] =
        await Promise.all([
      ...
      const candidates = this.engine.evaluate({
        ...
        facts: {
          ...
          totalWorkEntries: entries.filter(({ type }) => type === "work")
            .length,
          totalSessions,
          totalNotes,
          totalQuotes,
      ...
      return Object.freeze(await this.insertCandidates(candidates));
    } catch (error: unknown) {
      if (error instanceof InfrastructureError) throw error;
      throw new InfrastructureError("DATABASE_WRITE_FAILED", "save_milestone");
    }
  }
