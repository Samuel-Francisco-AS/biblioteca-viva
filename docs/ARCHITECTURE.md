...
React consulta `GetRoomProgress`, aceita ou rejeita `RoomRequested` e entrega um `RoomViewModel` sem conteúdo pessoal. `LibraryVisualHost` mantém um único `Phaser.Game`/canvas e atualiza a sala ativa na cena existente, preservando áudio, atmosfera, resize e lifecycle. ResidentCatalog mantém as cinco identidades fechadas, residência, perfil de diálogo, estados e âncoras sem tabela persistida. Residentes secundários são derivados de `highestReachedStage >= 2`; a bibliotecária permanece na Biblioteca Principal. `InitialLibraryScene` mantém um único residente secundário, scheduler compartilhado por cena e cleanup de renderer, scheduler e tweens na troca/dispose. A criatura não é residente: sua política pura escolhe deterministicamente uma sala elegível e usa a Biblioteca Principal como fallback.

O Phaser emite `ResidentInteracted` com apenas `residentId` e `roomId`; `DialogueService` continua sendo o único seletor e recebe fatos agregados de sala/período/sessões, sem conteúdo bibliográfico ou PII. Reduced motion não inicia o scheduler e mantém poses estáticas. Dexie permanece v5 e o backup permanece v3.
