...
Após o gate, o teste, o comparador HTML, a cópia Vite da variante e seus arquivos de suporte foram removidos do checkout. A variante e o script temporário permanecem somente no laboratório externo como evidência até F4-F. A conclusão vale exclusivamente para este Poly Haven e este experimento: 512×512 não se torna budget global, outros assets podem exigir outra resolução e a técnica não define Pipeline 3D definitivo.

## F4-E5 — consolidação e fechamento — concluída experimentalmente

F4-E consolidou uma evidência de processo, não um pipeline definitivo. O baseline E1 confirmou os quatro fixtures registrados e separou medidas exatas, métricas derivadas e estimativas: o corpus tem 5.901.424 bytes, 1.923 vértices, 1.104 triângulos, 66.240 bytes lógicos de geometria, 5.829.802 bytes de imagens codificadas e estimativa RGBA8 base de 16 MiB. E2 mostrou que Poly Haven é o outlier: 5.828.612 bytes (98,766% do corpus), dos quais 5.814.197 bytes de três imagens 1024×1024 (99,753% do próprio GLB); sua geometria tem apenas 182 triângulos e 12.676 bytes lógicos. KayKit, Kenney e Quaternius são pequenos nesta amostra e não justificaram experimento específico.

Por isso, não houve evidência para priorizar Draco, Meshopt, quantização ou simplificação geométrica neste corpus e nesta fase. Isso não os rejeita para todo o Biblioteca Viva: somente preserva que seu custo/complexidade de transformação e, quando aplicável, decoder não era justificado pelos dados medidos. KTX2/Basis também não foi testado nem aprovado ou rejeitado definitivamente; permanece possibilidade futura se volume real, payload de texturas e evidência posterior justificarem sua complexidade de transcoder, loader e compatibilidade.

E3/E4 testaram somente a hipótese de transformação offline de textura para Poly Haven. A variante laboratorial 512×512 reduziu o GLB de 5.828.612 para 711.352 bytes (-87,796%), as imagens codificadas de 5.814.197 para 696.943 bytes (-88,013%) e a estimativa RGBA8 base de 12 para 3 MiB (-75%). `POSITION`, `NORMAL`, `TEXCOORD_0` e índices permaneceram idênticos nos dados lógicos; hierarchy, transforms, bounding box, material, três imagens, papéis Base Color/normal/metallic-roughness combinado e o parse pelo `GLTFLoader` de `three@0.185.1` permaneceram equivalentes. Em particular, `metalnessMap === roughnessMap` continuou verdadeiro. A variante não introduziu extensão glTF, decoder, dependência ou alteração de runtime.

O gate humano A/B foi **PASS**, com a ressalva preservada: há leve desfoque perceptível em comparação próxima com o original 1024×1024. Para objetos menores e mais distantes na composição ortográfica/2.5D pretendida, a perda foi considerada irrisória e não bloqueante; identidade visual, material e leitura geral foram preservados. A aprovação não afirma equivalência pixel a pixel, ausência de perda ou qualidade universalmente igual.

### Conclusão experimental sobrevivente

Para assets cujo custo observado esteja concentrado em texturas superdimensionadas em relação ao tamanho e à apresentação reais do objeto, dimensionar a textura offline pode ser uma hipótese simples, isolável e de baixo custo operacional, capaz de reduzir payload e footprint estrutural estimado sem adicionar complexidade de runtime. A redução precisa preservar geometria e semântica de material, ser comparada objetivamente e receber gate visual quando puder afetar fidelity.

`512×512` **não é** budget global, formato obrigatório nem resolução universalmente recomendada. A resolução continua dependente de asset, distância, enquadramento e necessidade visual; budgets permanentes continuam abertos. O princípio que sobrevive é dimensionar textura conforme necessidade visual real e custo observado, não prescrever um número único.
