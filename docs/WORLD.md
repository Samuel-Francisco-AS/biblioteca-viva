...
### Livro procedural e layout BF-2B
...
`READING_AREA_BOOK_SLOTS` é derivado das dimensões internas das três variantes atuais: 32 posições nos quatro níveis úteis de `reading-shelf-01`, 30 nos cinco de `reading-shelf-02` e 27 nos três de `reading-shelf-03`. A ordem é a composição declarativa, níveis de baixo para cima e slots da esquerda para a direita; os 89 slots determinam a capacidade. A atribuição recebe somente `instanceId`, não reordena a entrada, rejeita IDs duplicados e devolve o excedente ordenado explicitamente. Slots e placements são locais ao wrapper estável da instância da estante: na BF-2C um livro será irmão da root de representação substituível, portanto BF-1D não poderá descartá-lo ao trocar a estante. São valores reconstruídos em memória, sem schema, tabela, backup, `PlacedObject`, `WorldStructureState` ou posição persistida. Nenhum `ReadingAreaBook` é anexado ao runtime normal, visível na rota ou selecionável no canvas nesta etapa; BF-2C–E permanecem futuras.
