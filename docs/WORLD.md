...
BF-3B projetará `LibraryEntry[]` em dados TypeScript puros, sem React, Three.js, Dexie, browser, `application` ou caso de uso. A entrada usa somente o contrato convencional já validado; a saída não carrega a entrada de domínio, posição, slot, mesh, root, URL ou dado de infraestrutura. Toda ocorrência exige `entryId` (o `id` convencional), `type`, `title`, `createdAt`, `modelTypeId` e `instanceId`. Os campos comuns efetivamente disponíveis — `updatedAt`, `revision`, `status`, `startedAt`, `completedAt`, `favorite` e `tagIds` — não são metadados mínimos da representação e não devem ser copiados por conveniência. Podem continuar disponíveis ao fluxo React por seu registro convencional, sem que a projeção invente valores.
...
A área de leitura e seus livros mantêm seus wrappers, bounds, câmera, enquadramento, seleção e capacidade próprios. Os cinco tipos ocuparão, no máximo, uma zona complementar provisória compatível com a mesma cena e fora dos bounds medidos das estantes, livros colocados e proxies técnicos; isso não cria cinco salas, coordenadas definitivas ou estado espacial. BF-3C avaliará como hipóteses mídia para filme/série, caderno para estudo, marcador neutro para atividade física e pasta para trabalho. As hipóteses não são arte final, não são obrigatórias e uma família geométrica só poderá servir a mais de um tipo se `modelTypeId`, rótulo e apresentação semântica continuarem inequivocamente distintos.

Antes de fixar qualquer número, BF-3C mede os bounds reais das fábricas escolhidas e declara slots finitos, posições finitas e espaçamento positivo entre volumes e limites da zona. O algoritmo puro deve: preservar a ordem recebida dentro da área/categoria; rejeitar `instanceId` repetido; provar ausência de interseção positiva entre bounds colocados, área de leitura e proxies relevantes; e derivar capacidade da quantidade efetiva de slots após essa geometria. Overflow conserva a ordem lógica, não recebe root, recurso, catálogo nem picking. A capacidade de 70 é exclusiva das estantes de livros e não participa desse cálculo. A inspeção humana dirigida no navegador, com câmera/enquadramento e seleção atuais, é obrigatória no gate C antes de qualquer conexão ao runtime; ela não é prova de Android, TalkBack ou performance física.
