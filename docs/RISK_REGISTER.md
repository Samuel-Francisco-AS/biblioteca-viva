...
| R-03 | perda ou evicção de dados | média | alto | portas, backup, restauração, migrações e teste Android | dado some após reinício/atualização | ativo |
...
| R-11 | backup importar conteúdo malicioso | baixa | alto | schema, limite, sanitização e transação | arquivo externo aceito sem validação | ativo |
