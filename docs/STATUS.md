...
**BF-2E/BF-2, BF-3A, BF-3B e BF-3C1/C2 passaram seus gates aplicáveis. BF-3C3 está EM ANDAMENTO e seu primeiro gate visual foi FAIL dirigido.** Próximo trabalho: substituir a hipótese de sala única por uma construção procedural conectada com cinco cômodos neutros, aumentar/redesenhar as representações conforme necessário para leitura visual, redistribuir os tipos apenas como configuração provisória da prévia e derivar novamente slots/capacidade. Os cômodos não podem carregar categoria, etiqueta ou restrição semântica; a solução deve preservar a possibilidade futura de o usuário reorganizar objetos entre ambientes. Depois, repetir o gate humano no navegador. **BF-3 não está concluída.** Continuam fora de C3: seleção/picking produtivos, `ThreeWorldRuntime`, React, `listLibraryEntries`, Dexie, persistência espacial, ferramenta de mover objetos, GLB e decoração final.
...
BF-3 📋 CONTRATO DOCUMENTADO — PROJEÇÃO PURA, C1 E C2 CONCLUÍDAS
  ...
  BF-3C ⏳ procedural e layout isolados — C1/C2 CONCLUÍDAS; C3 EM ANDAMENTO
    ...
    BF-3C3 ⏳ primeira prévia FAIL; hipótese multiambiente neutra autorizada
