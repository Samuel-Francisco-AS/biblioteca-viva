...
- A cena atual é um spike técnico com fixture, não a Biblioteca final nem arquitetura permanente de conteúdo. BF-1C/BF-1D permanecem aprovadas nos seus contratos de wrappers, seleção e substituição. BF-2A criou a projeção neutra e BF-2B a fábrica/layout isolados. **BF-2C está concluída tecnicamente:** `ReadingAreaBook` foi extraído para contrato sem domain/application/Three/infrastructure e entra opcionalmente como snapshot no runtime; `bookshelf` agora usa identidade estrita própria. Slots são derivados da variante atual (89 somente na configuração inicial) e BF-1D relayouta livros, reutilizando os visíveis, removendo/liberando overflow e recriando somente retornos. Livros são irmãos da representação da estante, selecionáveis pelo `instanceId`, e a seleção carrega `entryId`; F5 ignora o conteúdo BF. Não há query, React funcional, Dexie, navegação, schema, backup ou persistência espacial. BF-2D/E não foram iniciadas. Não há GLB produtivo, loader produtivo, cache, `PlacedObject`, `WorldStructureState` ou tabela espacial.
...
PRÓXIMO AUTORIZÁVEL: planejamento e implementação autorizada da BF-2D; nenhuma integração automática React/aplicação nesta etapa
```
