...
- **BF-3A está concluída no plano documental, BF-3B no escopo de projeção pura e BF-3C1 no escopo técnico isolado.** `projectLibraryWorldEntries()` recebe somente `readonly LibraryEntry[]` e publica snapshot imutável com seis categorias na ordem de `ENTRY_TYPES`; `book` reutiliza integralmente `projectReadingAreaBooks()` e `ReadingAreaBook`, sem `type` ou `createdAt`, enquanto `movie`, `series`, `study`, `physical_activity` e `work` preservam apenas seus metadados BF-3A, discriminantes e identidades semânticas. A projeção ordena por `createdAt`/`id`, rejeita `entryId` duplicado no conjunto completo e confere `instanceId` único sem mutar a entrada. A rota Biblioteca usará somente em BF-3E uma única `listLibraryEntries` por montagem normal — não duas consultas, pois `listBookEntries` já consulta o agregado e filtra livros —, mantendo `listBookEntries`, `projectReadingAreaBooks()` e `reading-book:${entryId}` intactos. A C1 adicionou somente cinco fábricas Three isoladas em `libraryRecordRepresentations.ts`, com entrada estreita (`entryId`, `instanceId`, `modelTypeId`) derivada dos contratos BF-3B, root em identidade apoiada em `Y=0`, dimensões reais verificadas e recursos próprios por chamada. Painel audiovisual (filme), três casos de mídia (série), caderno (estudo), marcador neutro de percurso (atividade física) e pasta/arquivos (trabalho) medem, em largura × altura × profundidade, 0,720 × 0,480 × 0,240 m; 0,660 × 0,500 × 0,200 m; 0,560 × 0,132 × 0,400 m; 0,460 × 0,590 × 0,400 m; e 0,600 × 0,300 × 0,400 m. Não houve layout, slots, capacidade, coordenada global, runtime, interface React, consulta na página, teste de produção ou APK; as formas seguem hipóteses e não receberam gate visual humano.
...
**BF-2E está concluída no escopo acordado, BF-2 está concluída no escopo técnico-funcional e físico dirigido, BF-3A passou o gate documental, BF-3B passou o gate de projeção pura e BF-3C1 passou o gate técnico isolado.** O próximo autorizável é **BF-3C2 — layout determinístico, bounds e overflow**: deve consumir as dimensões efetivas C1, declarar a zona complementar e slots finitos, provar espaçamento/ausência de colisão com área de leitura, livros e proxies, e preservar overflow ordenado sem criar roots. BF-3C3 continuará responsável pela prévia isolada e pela decisão visual humana explícita; BF-3C não está concluída. O contrato integral está em `docs/ROADMAP.md`. Não autoriza integração com `ThreeWorldRuntime`, interface React, `listLibraryEntries`, nova dependência ou persistência espacial. A estratégia de consulta para BF-3E permanece decidida, mas não deve ser implementada antecipadamente: uma `listLibraryEntries` por montagem normal da Biblioteca, sem sincronização live e sem F5.
...
BF-3 📋 CONTRATO DOCUMENTADO — PROJEÇÃO PURA E C1 CONCLUÍDAS
  ...
  BF-3C ⏳ procedural e layout isolados — C1 CONCLUÍDA; C2/C3 PENDENTES
