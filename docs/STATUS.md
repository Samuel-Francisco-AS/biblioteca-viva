...
**BF-2E/BF-2, BF-3A, BF-3B e BF-3C1/C2 passaram seus gates aplicáveis.** O próximo autorizável é **BF-3C3 — prévia isolada, gate visual humano e fechamento**: deve usar conteúdo fictício e a câmera/renderer existentes somente em instrumento temporário, avaliar distinção, proporções, legibilidade, enquadramento e oclusões, registrar decisão humana explícita e remover o instrumento. **BF-3 não está concluída.** A C2 não autoriza integração com `ThreeWorldRuntime`, interface React, `listLibraryEntries`, nova dependência ou persistência espacial. Riscos abertos para C3: envelopes/ausência de colisão não provam legibilidade do caderno de 0,132 m, oclusão ortográfica, selecionabilidade percebida, ergonomia Android ou desempenho; TalkBack permanece dívida obrigatória. A estratégia BF-3E continua posterior: uma `listLibraryEntries` por montagem normal, sem sincronização live e sem F5.
...
BF-3 📋 CONTRATO DOCUMENTADO — PROJEÇÃO PURA, C1 E C2 CONCLUÍDAS
  ...
  BF-3C ⏳ procedural e layout isolados — C1/C2 CONCLUÍDAS; C3 PENDENTE
