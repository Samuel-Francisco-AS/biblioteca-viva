...
- A cena atual é um spike técnico com fixture, não a Biblioteca final nem arquitetura permanente de conteúdo. BF-1C foi tecnicamente concluída e versionada em `34015b6`, mas a inspeção humana posterior apontou artefatos de leitura nas estantes. BF-1C-FIX confirmou faces coincidentes entre fundo/prateleiras da fábrica anterior e uma colisão estreita entre a variante larga e um banco F1; não confirmou mudança de câmera, projeção ou viewport causada pela rolagem. A fábrica agora separa fundo, prateleiras e molduras sem sobreposição de volume, cria três variantes declarativas (`reading-balanced`, `reading-dark-tall`, `reading-light-wide`) e move somente `reading-shelf-03` para `[3, 0, -2.1]`, afastando-a do proxy. Identidade, catálogo, seleção, destaque, ownership e F5 permanecem como BF-1C. A inspeção humana visual corretiva está **PENDENTE**; não há aprovação de arte, Android físico ou TalkBack. O Pipeline 3D v1 conserva seu contrato documental e o candidato `bookshelf` tem proveniência preservada, mas ainda não há fonte canônica normalizada, GLB ou asset produtivo, loader produtivo, catálogo real, substituição dinâmica, persistência espacial, `PlacedObject`, `WorldStructureState` ou tabela espacial.
