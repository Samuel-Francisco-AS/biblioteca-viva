...
- A cena atual é um spike técnico com fixture, não a Biblioteca final nem arquitetura permanente de conteúdo. BF-1C anexa estaticamente à experiência normal a composição BF-1B de três estantes em `[-3, 0, -2]`, `[0, 0, -2]` e `[3, 0, -2]`; seus IDs lógicos `reading-shelf-01`–`03` entram no catálogo já em `mount()`, têm rótulos legíveis, seleção por canvas e pelos controles React, e usam o destaque existente. As roots procedurais têm prioridade somente quando uma interseção as alcança, para manter a seleção de uma estante mesmo onde um proxy F1 sobreposto a ocluiria; fora dessa sobreposição, o picking técnico conserva a regra vigente. A montagem é o único owner da root de composição e a libera por `disposeObjectTree()` no descarte terminal ou em falha durante a montagem; composições/remontagens continuam com recursos independentes. A presença explícita de `performanceScenario` mantém F5 estritamente diagnóstico: não anexa essa composição e preserva seu catálogo, contagens e corpus. Não há associação a registros. A oclusão visual potencial dos proxies F1 permanece observação para o gate visual; o layout/fixture não foi alterado. O Pipeline 3D v1 conserva seu contrato documental e o candidato `bookshelf` tem proveniência preservada, mas ainda não há fonte canônica normalizada, GLB ou asset produtivo, loader produtivo, catálogo real, substituição dinâmica, persistência espacial, `PlacedObject`, `WorldStructureState` ou tabela espacial.
...
**BF-1C está concluída tecnicamente.** A composição valida `instanceId` duplicado ou vazio e coordenadas não finitas antes de alocar recursos; na experiência normal ela é anexada uma vez, preserva a root local da fábrica e fornece três roots selecionáveis por instância, sem colisão com os selecionáveis técnicos. F5 continua sem composição procedural. A cobertura automatizada verifica catálogo imediato, seleção React/Three, partes internas, destaque, descarte, falha/remontagem e isolamento; a validação humana visual, assistiva e física continua pendente. O próximo checkpoint autorizado é **BF-1D — substituição, descarte e gate**; BF-1 inteira não está concluída. P3D-B1 permanece preservada como registro documental de `bookshelf`; P3D-B2–F estão adiados e só retomam se houver necessidade concreta de ingestão ou substituição por assets GLB definitivos. A FUNDAÇÃO permanece concluída tecnicamente/arquiteturalmente; a dívida humana de TalkBack continua obrigatória antes do fechamento do primeiro recorte real e de beta/release aplicável. A fixture continua técnica, sem mundo final ou persistência espacial.
