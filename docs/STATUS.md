...
A primeira W3-A visual foi reprovada no Moto G06: o compositor por limites de piso tratou cantos de braços longos como juntas unitárias, deixando braços externos e segmentos sobrepostos. O pipeline RGBA, catálogo, IDs e fallback foram preservados, mas aquele compositor não é usado na execução normal. O Processo 1 cria um único cômodo persistente 12×10 (x=3..14, y=4..13): pisos são células, paredes/cantos/porta são arestas explícitas. Dexie v7 adiciona `worldStructures`; backup v5 preserva o estado e aceita v1–v4. Não há editor, inventário UI ou porta vertical. Validação visual/física no Moto G06 continua pendente.
