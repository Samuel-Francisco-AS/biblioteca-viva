...
- **BF-3A está concluída no plano documental, BF-3B no escopo de projeção pura e BF-3C1/C2 no escopo técnico isolado. BF-3C3 permanece em andamento.** A C1 criou cinco fábricas procedurais e a C2 provou uma primeira distribuição de sala única com 37 slots derivados, sem colisões geométricas. A primeira prévia humana dessa hipótese foi tecnicamente utilizável e confirmou pan/pinch, mas recebeu **FAIL visual dirigido** porque as novas representações ficaram pequenas demais e não eram reconhecíveis no enquadramento normal; seleção/picking não foi implementado por pertencer à BF-3D. A capacidade 37 permanece somente como evidência da hipótese C2, não como meta. A segunda hipótese C3 melhorou a legibilidade das representações, mas recebeu **FAIL visual dirigido na planta**: o retângulo subdividido não transmitiu um lugar de vivência. A hipótese 3 substitui essa planta na prévia isolada por um núcleo central e quatro salas periféricas deslocadas, ligados por passagens curtas e inspirados na topologia da referência 2D. Os cinco cômodos continuam **semanticamente neutros**; os nomes/temas da imagem 2D são vestígios superados e não compõem o contrato atual. A distribuição inicial de leitura, filme+série, estudo, atividade física e trabalho entre esses espaços serve apenas à prévia; nenhum cômodo terá categoria, etiqueta ou regra que vincule seu uso a um tipo. O desenho deve permanecer compatível com a direção futura de permitir que o usuário mova objetos e reorganize o ambiente, embora movimento e persistência espacial continuem fora de C3.
...
**BF-2E/BF-2, BF-3A, BF-3B e BF-3C1/C2 passaram seus gates aplicáveis. BF-3C3 está EM ANDAMENTO.** Próximo trabalho: abrir `bf3c3-preview.html` via `npm run dev` (`http://localhost:5173/bf3c3-preview.html`) e executar novo gate humano da hipótese 3 para planta, circulação, escala, oclusões e enquadramento dos cinco cômodos semanticamente neutros. A prévia contém 13 placements derivados e overflow correto; a distribuição é temporária. **BF-3C3 e BF-3C não estão concluídas.** Seleção/picking BF-3D, integração produtiva, persistência espacial, edição de planta, GLB e decoração final permanecem fora deste checkpoint.
...
BF-3 📋 CONTRATO DOCUMENTADO — PROJEÇÃO PURA, C1 E C2 CONCLUÍDAS
  ...
  BF-3C ⏳ procedural e layout isolados — C1/C2 CONCLUÍDAS; C3 EM ANDAMENTO
    ...
    BF-3C3 ⏳ hipóteses 1/2 reprovadas; hipótese 3 pronta, gate humano pendente
