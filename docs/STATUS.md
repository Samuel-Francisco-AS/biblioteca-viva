...
- **BF-3A está concluída no plano documental, BF-3B no escopo de projeção pura e BF-3C1/C2 no escopo técnico isolado.** `projectLibraryWorldEntries()` recebe somente `readonly LibraryEntry[]` e publica snapshot imutável com seis categorias na ordem de `ENTRY_TYPES`; `book` reutiliza integralmente `projectReadingAreaBooks()` e `ReadingAreaBook`, sem `type` ou `createdAt`, enquanto `movie`, `series`, `study`, `physical_activity` e `work` preservam apenas seus metadados BF-3A, discriminantes e identidades semânticas. A projeção ordena por `createdAt`/`id`, rejeita `entryId` duplicado no conjunto completo e confere `instanceId` único sem mutar a entrada. A C1 adicionou cinco fábricas Three isoladas com os envelopes reais já registrados. A C2 extraiu esses envelopes para autoridade renderer-free e criou `libraryRecordLayout.ts`: zona complementar central/frontal `X=[-3.2, 2.8]`, `Y=[0, 0.7]`, `Z=[1.0, 4.5]`, cinco faixas semânticas e capacidade derivada de **37** slots (filme 6, série 7, estudo 8, atividade física 9, trabalho 7). A atribuição pura preserva `modelTypeId`/`instanceId`/`entryId`, posição/slot determinísticos e overflow ordenado, rejeita configurações ou duplicatas inválidas e não cria roots. Bounds reais C1, volumes entre si, as três estantes BF-1, os 70 livros BF-2 e paredes/proxies técnicos modelados passaram no teste `Box3`; apoio no piso é deliberadamente excluído de colisão. Não houve runtime, interface React, consulta na página, alteração de câmera/cena, schema, backup ou APK. A primeira parte da C3 preparou uma prévia temporária em `bf3c3-preview.html`: ela projeta apenas entradas convencionais sintéticas determinísticas pela fronteira BF-3B, usa as fábricas C1 e placements C2 reais, reúne cena de referência, estantes BF-1 e 18 livros fictícios BF-2, e libera suas roots ao trocar cenário ou desmontar. Os cenários são A (5 placements), B (37) e C (37, com overflow filme 2 e estudo 2); o painel não cria roots para excedentes. A página é separada do `index.html` e não toca `LibraryPage`, `WorldHost`, `ThreeWorldRuntime`, consultas, Dexie, F1 ou F5. Ainda falta exclusivamente o gate visual humano; não houve aprovação de legibilidade, oclusão, navegação percebida, Android ou desempenho.
...
**BF-2E/BF-2, BF-3A, BF-3B e BF-3C1/C2 passaram seus gates aplicáveis. BF-3C3 está EM ANDAMENTO: a prévia isolada está pronta e aguarda inspeção humana.** Para abrir, executar `npm run dev` e acessar `http://localhost:5173/bf3c3-preview.html`; os botões A, B e C alternam baixa densidade, capacidade completa e overflow. A navegação reutiliza os gestos atuais de pan, wheel focal e pinch, mas não oferece seleção/picking dos novos tipos nem prova equivalência ergonômica a aparelho físico. Após o relatório humano explícito, retomar na C3: registrar achados, corrigir somente se indicado, repetir a inspeção afetada e, apenas após aprovação, remover `bf3c3-preview.html` e os módulos `bf3c3Preview*`, executar a regressão proporcional e fechar a documentação. **BF-3 não está concluída.** A C2/C3 não autoriza integração com `ThreeWorldRuntime`, interface React, `listLibraryEntries`, nova dependência ou persistência espacial. Riscos abertos: envelopes/ausência de colisão não provam legibilidade do caderno de 0,132 m, oclusão ortográfica, selecionabilidade percebida, ergonomia Android ou desempenho; TalkBack permanece dívida obrigatória. A estratégia BF-3E continua posterior: uma `listLibraryEntries` por montagem normal, sem sincronização live e sem F5.

A sequência vigente é C3 (prévia, gate visual humano e fechamento), D (runtime/seleção), E (React/aplicação e fluxo funcional) e F (regressão, APK, validação física dirigida e fechamento). A aprovação visual humana na C3 é ponto de decisão obrigatório; não iniciar D antes dela. Preservar o livro, seus 70 slots/capacidade derivados, a lista semântica e seleção por `entryId`; não promover fixture/diagnóstico a produção. TalkBack continua obrigatório antes do fechamento do primeiro recorte real e de beta/release aplicável e não recebeu PASS. P3D-B1 permanece registro documental de `bookshelf`; P3D-B2–F seguem adiados até necessidade concreta de GLB definitivo. A cena continua provisória, sem mundo final ou persistência espacial.
...
BF-3 📋 CONTRATO DOCUMENTADO — PROJEÇÃO PURA, C1 E C2 CONCLUÍDAS
  ...
  BF-3C ⏳ procedural e layout isolados — C1/C2 CONCLUÍDAS; C3 EM ANDAMENTO
