...
- A cena atual é um spike técnico com fixture, não a Biblioteca final nem arquitetura permanente de conteúdo. BF-1A implementou a fábrica procedural isolada de `bookshelf`; BF-1B a reúne em composição declarativa de três instâncias com wrappers de posicionamento determinístico e recursos independentes entre composições. A composição não está na cena, não participa de seleção e não associa registros. O Pipeline 3D v1 conserva seu contrato documental e o candidato `bookshelf` tem proveniência preservada, mas ainda não há fonte canônica normalizada, GLB ou asset produtivo, loader produtivo, catálogo real, substituição dinâmica, persistência espacial, `PlacedObject`, `WorldStructureState` ou tabela espacial.
...
**BF-1B está concluída.** A composição recebe definições sem tipos Three, valida `instanceId` duplicado antes de alocar recursos e cria três estantes `reading-shelf-01`–`03` nas posições provisórias `[-3, 0, -2]`, `[0, 0, -2]` e `[3, 0, -2]`. A fábrica permanece em identidade dentro de wrapper de instância; chamadas e composições independentes não compartilham recursos, e `disposeObjectTree()` libera a root de composição com deduplicação intrárvore. O próximo checkpoint autorizado é **BF-1C — integração com runtime e seleção**. P3D-B1 permanece preservada como registro documental de `bookshelf`; P3D-B2–F estão adiados e só retomam se houver necessidade concreta de ingestão ou substituição por assets GLB definitivos. A FUNDAÇÃO permanece concluída tecnicamente/arquiteturalmente; a dívida humana de TalkBack continua obrigatória antes do fechamento do primeiro recorte real e de beta/release aplicável. A fixture continua técnica, sem mundo final ou persistência espacial.
