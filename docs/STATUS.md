...
- **BF-3A está concluída exclusivamente no plano documental.** O contrato vigente em `WORLD.md` preserva BF-2 e fixa, para `movie`, `series`, `study`, `physical_activity` e `work`, identidades semânticas e namespaces sem colisão, metadados realmente disponíveis, ordenação/duplicidade/snapshot, overflow e fronteiras de layout, runtime, React, lifecycle e F5. A rota Biblioteca usará em BF-3E uma única `listLibraryEntries` por montagem normal — não duas consultas, pois `listBookEntries` já consulta o agregado e filtra livros —, mantendo `listBookEntries`, `projectReadingAreaBooks()` e `reading-book:${entryId}` intactos. Não houve implementação de projeção, geometria, layout, runtime, interface, teste de produção ou APK; as formas candidatas continuam hipóteses e nenhuma capacidade global ou coordenada foi definida.
...
**BF-2E está concluída no escopo acordado, BF-2 está concluída no escopo técnico-funcional e físico dirigido, e BF-3A passou o gate exclusivamente documental.** A próxima execução autorizável é **BF-3B — projeção neutra dos cinco tipos restantes**: implementar somente contratos e testes puros a partir de `LibraryEntry[]`, conforme `WORLD.md`, preservando a projeção BF-2. Não autoriza geometria, layout, runtime, interface React, consulta na página, nova dependência ou persistência espacial. A estratégia de consulta para BF-3E já está decidida, mas não deve ser implementada antecipadamente: uma `listLibraryEntries` por montagem normal da Biblioteca, sem sincronização live e sem F5.
