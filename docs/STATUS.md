...
- A cena atual é um spike técnico com fixture, não a Biblioteca final nem arquitetura permanente de conteúdo. BF-1C foi tecnicamente concluída e versionada em `34015b6`; BF-1C-FIX confirmou faces coincidentes entre fundo/prateleiras da fábrica anterior e uma colisão estreita entre a variante larga e um banco F1. O gate visual humano posterior ao commit `493c887` foi **PASS**: as três estantes e variantes ficaram legíveis, sem as deformidades, linhas ou perdas de definição anteriores. Isso aprova somente a correção visual no navegador, não arte final, TalkBack ou Android físico. BF-1D está tecnicamente concluída: na experiência normal, troca de modo síncrono somente uma representação `bookshelf` pelo `instanceId`, preserva identidade lógica, posição, wrapper selecionável, catálogo e demais instâncias; prepara a sucessora, anexa-a, transfere ownership e libera a predecessora uma vez. A seleção ativa mantém ID/rótulo/datasets e recria seu `Box3Helper` para os novos bounds. F5 não recebe composição nem operação de troca. Os gates dirigidos e integrados passaram: fábrica/composição/runtime/host, fronteiras, 546 testes Vitest, format, lint, typecheck, build e 6 E2E da rota Biblioteca; o build conservou somente o aviso conhecido de chunks acima de 500 kB. BF-2A está concluída: há projeção neutra e determinística de `BookEntry` para ocorrências futuras `book-volume`. BF-2B está concluída: existe fábrica procedural isolada de livro, com `entryId` obrigatório no contrato estreito, e slots determinísticos das três estantes com capacidade/overflow explícitos. Os slots são locais ao wrapper estável, não à root de representação que BF-1D descarta; nenhum livro real é anexado ao Three/runtime, aparece na rota, recebe seleção canvas ou tem posição persistida. BF-2C–E permanecem não iniciadas. Não há GLB produtivo, loader produtivo, cache, `PlacedObject`, `WorldStructureState` ou tabela espacial.
...
BF-2 ⏳ EM ANDAMENTO
  ...
  BF-2E ⏳ gate integrado e fechamento da BF-2 — NÃO INICIADA
PRÓXIMO AUTORIZÁVEL: planejamento e implementação autorizada da BF-2C; nenhuma integração automática ao runtime
```
